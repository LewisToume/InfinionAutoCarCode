...
/*------------------------------------------------------Macros-------------------------------------------------------*/
/*********************************************************************************************************************/
#define ISR_PRIORITY_VSYNC_INT0        56                       /* Define the SCU ERU interrupt priority                */
#define PIX_REQ                        &IfxScu_REQ7_P00_4_IN    /* External request pin                                 */
#define VSYNC_REQ                      &IfxScu_REQ16_P15_1_IN
 ...
 */
IFX_INTERRUPT(VSYNC_Int0_Handler, 0, ISR_PRIORITY_VSYNC_INT0);


/* Interrupt Service Routine */
void VSYNC_Int0_Handler(void)
{
    triggerDMATransfer();
}
...
void initPeripheralsAndERU(void)
{
    /*********************************像素时钟中断*************************************/
    /* Trigger pin */
    g_ERUconfig.reqPin = PIX_REQ; /* Select external request pin */
    ...
    IfxScuEru_enableFallingEdgeDetection(g_ERUconfig.inputChannel);          /* Interrupt triggers on
    ...
    IfxSrc_init(g_ERUconfig.src, IfxSrc_Tos_dma, DMA_CHANNEL_ID);//中断优先级需要与DMA通道一致，同时中断请求是交给DMA的ICU处理
    IfxSrc_enable(g_ERUconfig.src);

    /*********************************场同步中断*************************************/
    /* Trigger pin */
    g_ERUconfig.reqPin = VSYNC_REQ; /* Select external request pin */
    ...
    IfxScuEru_enableRisingEdgeDetection(g_ERUconfig.inputChannel);          /* Interrupt triggers on
                                                                               rising edge (Register RENx) and  */
    /* Signal destination */
    g_ERUconfig.outputChannel = IfxScuEru_OutputChannel_1;                  /* OGU channel 0                    */
    /* Event from input ETL0 triggers output OGU0 (signal TRx0) */
    g_ERUconfig.triggerSelect = IfxScuEru_InputNodePointer_1;
    ...
    /* Get source pointer depending on outputChannel (SRC_SCUERU0 for outputChannel0) */
    g_ERUconfig.src = &MODULE_SRC.SCU.SCU.ERU[(int) g_ERUconfig.outputChannel % 4];
    IfxSrc_init(g_ERUconfig.src, IfxSrc_Tos_cpu0, ISR_PRIORITY_VSYNC_INT0);
    IfxSrc_enable(g_ERUconfig.src);
}
