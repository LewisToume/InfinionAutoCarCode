...
IFX_INTERRUPT(DMA_Int0_Handler, 0, DMA_PRIORITY);

void DMA_Int0_Handler(void)
{
    Camera_Flag++;
    if(Camera_Flag == 2) {
        IfxDma_disableChannelTransaction(&MODULE_DMA, DMA_CHANNEL_ID);
    } else {
        IfxDma_setChannelDestinationAddress(&MODULE_DMA, DMA_CHANNEL_ID, &Image_Data[60][0]);
    }
}
//场同步信号中断
void triggerDMATransfer(void)
{
    if(Camera_Flag == 0) {
        IfxDma_setChannelDestinationAddress(&MODULE_DMA, DMA_CHANNEL_ID, Image_Data);
        IfxDma_enableChannelTransaction(&MODULE_DMA,DMA_CHANNEL_ID);
    }
}
/*!
 * @brief    摄像头初始化
...
void CAMERA_Init (unsigned char fps)
{
    ...
    /* 初始化摄像头 数据IO */
    IfxPort_setPinMode(&MODULE_P02,0, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,1, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,2, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,3, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,4, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,5, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,6, IfxPort_Mode_inputPullDown);
    IfxPort_setPinMode(&MODULE_P02,7, IfxPort_Mode_inputPullDown);
...
void initDMA(void)
{
    ...
    /*******传输设置************/
    g_DMA.dmaChNCfg.transferCount = (uint16)DATA_ARRAY_LENGTH;  //DMA一次传输的次数，最大16385次（14位），总共需要传120x188=22560，因此分两次传输，一次传11280个
    g_DMA.dmaChNCfg.requestMode = IfxDma_ChannelRequestMode_oneTransferPerRequest;//每次收到传输请求后只搬运一次
    g_DMA.dmaChNCfg.moveSize = IfxDma_ChannelMoveSize_8bit;//传输的位宽，摄像头一次输出1个8位的灰度像素
    g_DMA.dmaChNCfg.operationMode = IfxDma_ChannelOperationMode_continuous;//指定数据长度传完后是继续传输
    g_DMA.dmaChNCfg.hardwareRequestEnabled = TRUE;//启动硬件请求，即像素时钟请求
    /*******传输源设置************/
    g_DMA.dmaChNCfg.sourceAddress = (uint32)(&MODULE_P02.IN.U);//摄像头的数据接口
    g_DMA.dmaChNCfg.sourceCircularBufferEnabled = TRUE;
    g_DMA.dmaChNCfg.sourceAddressCircularRange = IfxDma_ChannelIncrementCircular_none;
    /*******传输目的地设置************/
    g_DMA.dmaChNCfg.destinationAddress = (uint32)Image_Data;//存储图像的变量地址
    g_DMA.dmaChNCfg.destinationCircularBufferEnabled = FALSE;
    g_DMA.dmaChNCfg.destinationAddressCircularRange = IfxDma_ChannelIncrementCircular_32768;
    /*******DMA中断设置************/
    g_DMA.dmaChNCfg.channelInterruptEnabled = TRUE;
    g_DMA.dmaChNCfg.channelInterruptControl = IfxDma_ChannelInterruptControl_thresholdLimitMatch;//传输完成后产生中断
    g_DMA.dmaChNCfg.channelInterruptPriority = DMA_PRIORITY;
    g_DMA.dmaChNCfg.channelInterruptTypeOfService = IfxSrc_Tos_cpu0;

    g_DMA.dmaChNCfg.channelId = DMA_CHANNEL_ID;//指定DMA通道，需要与像素时钟中断优先级一致
