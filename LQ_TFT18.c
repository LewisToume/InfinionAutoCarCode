...
//龙邱TFT SPI口模块管脚顺序，TFT分辨率：128x160
#define TFTSPI_CS       &MODULE_P20,14     // CS管脚 默认拉低，可以不用
#define TFTSPI_SCK      &MODULE_P20,11     // SPI SCK管脚
#define TFTSPI_SDI      &MODULE_P20,10     // SPI MOSI管脚
#define TFTSPI_DC       &MODULE_P20,12     // D/C管脚
#define TFTSPI_RST      &MODULE_P20,13     // RESET管脚
...
#define TFTSPI_CS_H     IfxPort_setPinState(TFTSPI_CS, IfxPort_State_high)      /*!< CS管脚 */
#define TFTSPI_SCK_H    IfxPort_setPinState(TFTSPI_SCK, IfxPort_State_high)     /*!< SCL管脚 */
#define TFTSPI_SDI_H    IfxPort_setPinState(TFTSPI_SDI, IfxPort_State_high)     /*!< SDI管脚 */
#define TFTSPI_DC_H     IfxPort_setPinState(TFTSPI_DC, IfxPort_State_high)      /*!< DC管脚 */
#define TFTSPI_RST_H    IfxPort_setPinState(TFTSPI_RST, IfxPort_State_high)     /*!< RST管脚 */

#define TFTSPI_CS_L     IfxPort_setPinState(TFTSPI_CS, IfxPort_State_low)     /*!< CS管脚 */
#define TFTSPI_SCK_L    IfxPort_setPinState(TFTSPI_SCK, IfxPort_State_low)     /*!< SCL管脚 */
#define TFTSPI_SDI_L    IfxPort_setPinState(TFTSPI_SDI, IfxPort_State_low)     /*!< SDI管脚 */
#define TFTSPI_DC_L     IfxPort_setPinState(TFTSPI_DC, IfxPort_State_low)     /*!< DC管脚 */
#define TFTSPI_RST_L    IfxPort_setPinState(TFTSPI_RST, IfxPort_State_low)     /*!< RST管脚 */
...
void TFTSPI_Init(unsigned char type)
{
  //-----端口初始化----//
    TFTSPI_CS_L;
    IfxPort_setPinMode(TFTSPI_CS, IfxPort_Mode_outputPushPullGeneral);
    IfxPort_setPinPadDriver(TFTSPI_CS, IfxPort_PadDriver_cmosAutomotiveSpeed2);
    TFTSPI_DC_L;
    IfxPort_setPinMode(TFTSPI_DC, IfxPort_Mode_outputPushPullGeneral);
    IfxPort_setPinPadDriver(TFTSPI_DC, IfxPort_PadDriver_cmosAutomotiveSpeed2);
    TFTSPI_RST_L;
    IfxPort_setPinMode(TFTSPI_RST, IfxPort_Mode_outputPushPullGeneral);
    IfxPort_setPinPadDriver(TFTSPI_RST, IfxPort_PadDriver_cmosAutomotiveSpeed2);
...
#else
	TFTSPI_SCK_L;
	IfxPort_setPinMode(TFTSPI_SCK, IfxPort_Mode_outputPushPullGeneral);
	IfxPort_setPinPadDriver(TFTSPI_SCK, IfxPort_PadDriver_cmosAutomotiveSpeed2);
	TFTSPI_SDI_L;
	IfxPort_setPinMode(TFTSPI_SDI, IfxPort_Mode_outputPushPullGeneral);
	IfxPort_setPinPadDriver(TFTSPI_SDI, IfxPort_PadDriver_cmosAutomotiveSpeed2);
...
#else           //下面是9163驱动IC
  TFTSPI_Write_Cmd(0x3a);                   //每次传送16位数据(VIPF3-0=0101)，每个像素16位(IFPF2-0=101) RGB565格式
...
void TFTSPI_Set_Pos(unsigned char xs,unsigned char ys,unsigned char xe,unsigned char ye)
{
  ...
  TFTSPI_Write_Byte(ys+2);			//列起始坐标低8位
  TFTSPI_Write_Byte(0x00);		//列终止坐标高8位，始终为零
  TFTSPI_Write_Byte(ye+2);			//列终止坐标低8位
...
{     
  "北京龙邱丘智能科技淘宝店液晶单片机智能车器人启动中电压量不足请稍候，！：℃攻击娱乐模式普通格斗对话俯卧撑倒立握手敬礼跳舞蹈运动蓝精灵太极街舞温度—潘子"
};
...
const unsigned char hanzi16x16[] = {
  ...
  0x04,0x92,0x04,0x54,0x21,0xFF,0x10,0x54,0x10,0x92,0x05,0x01,0x04,0xFE,0x08,0x92,
  0x38,0x92,0x08,0xFE,0x08,0x92,0x08,0x92,0x08,0xFE,0x00,0x82,0x00,0x00,0x00,0x00, // 潘
  0x00,0x04,0x00,0x08,0x00,0x10,0x00,0x60,0x00,0x40,0x3F,0xFF,0x00,0x40,0x00,0x40,
  0x00,0x40,0x00,0x40,0x00,0x40,0x00,0x40,0x01,0x40,0x00,0x80,0x00,0x00,0x00,0x00, // 子
...
{
    TFTSPI_Show_Pic2(xs,ys,60,60,AyanamiRei_24bit);       //80,gImage_ftflogo);     //
}

void TFTSPI_Show_AyanamiEyeClosed(unsigned char xs,unsigned char ys)
{
    TFTSPI_Show_Pic2(xs, ys, 60, 60, AyanamiReiEyeClosed_24bit);
}
...
void Test_TFT18(void)
{
  ...
  TFTSPI_P8X16Str(0,1,"Hands off Ayanami!",u16WHITE,u16BLACK);		//字符串显示
  waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, 1000));        //延时等待
  char txt[32];
...
//      TFTSPI_P8X16Str(0, 6, txt, u16RED, u16BLUE);              //将txt中 内容显示出来
      waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, 500));              //延时等待
      count++;
   }
}//TEST()
